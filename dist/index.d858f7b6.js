...
for (const key of keys13)values13.push(apartment13[key]);
...
function countTotalSalary(salaries) {
    ...
    for (const key of keys16)totalSalary += salaries[key];
    return totalSalary;
}
...
function getAllPropValues(propName) {
    ...
    for (const product of products19){
        ...
        for (const property of rowsProperties)if (property === propName) returnTab.push(product[property]);
    }
    return returnTab;
}
...
function calculateTotalPrice(productName) {
    ...
    for (const product of products20){
        const rowsProperties = Object.keys(product);
        for (const property of rowsProperties)if (product[property] === productName) cost = product.price * product.quantity;
    }
    return cost;
}
...
function calculateMeanTemperature(forecast) {
    const { today: { low: todayLow, high: todayHigh, icon: todayIcon = "https://www.flaticon.com/svg/static/icons/svg/861/861059.svg" }, tomorrow: { low: tomorrowLow, high: tomorrowHigh, icon: tomorrowIcon = "https://www.flaticon.com/svg/static/icons/svg/2204/2204346.svg" } } = forecast;
...
function addOverNum(...args) {
    let total = 0;
    for (const arg of args)if (args[0] < arg) total += arg;
    return total;
...
function findMatches(...args) {
    ...
    for(let i = 0; i < args[0].length; i++){
        ...
        for(let j = 1; j < args.length; j++)if (firstTableNum === args[j]) matches.push(args[j]);
    }
    return matches;
}
